 ...
 * \note This function is `[[nodiscard]]` and `noexcept`.
 */
[[nodiscard]] auto strip_comment(std::string_view text) noexcept
    -> std::string_view;
 ...
 * destroyed or modified. Store the result only for immediate parsing.
 * \note This function is `[[nodiscard]]`.
 */
[[nodiscard]] auto split_whitespace_view(std::string_view text)
    -> std::vector<std::string_view>;

/**
 * \brief Splits a character sequence into owning whitespace-delimited tokens.
 *
 * \param[in] text Input character sequence.
 * \return Owning tokens whose lifetime is independent of `text`.
 * \note Prefer this function for public APIs or data retained beyond the
 * current expression.
 * \note This function is `[[nodiscard]]`.
 */
[[nodiscard]] auto split_whitespace_copy(std::string_view text)
 ...
 * \param[in] text Input character sequence.
 * \return Token views pointing into `text`.
 * \warning Kept for source compatibility; prefer `split_whitespace_view()`
 * when view lifetimes are intentional or `split_whitespace_copy()` for
 * owning tokens.
 * \note This function is `[[nodiscard]]`.
 */
[[nodiscard]] auto split_whitespace(std::string_view text)
    -> std::vector<std::string_view>;
 ...
 * \note This function is `[[nodiscard]]` and `noexcept`.
 */
[[nodiscard]] auto count_char(std::string_view text, char value) noexcept
    -> std::size_t;
 ...
 * \note This function is `[[nodiscard]]` and `noexcept`.
 */
[[nodiscard]] auto count_substring(std::string_view text,
 ...
 * \note This function is `[[nodiscard]]` and `noexcept`.
 */
[[nodiscard]] auto count_substring_case_insensitive(
    std::string_view text, std::string_view needle) noexcept -> std::size_t;
 ...
 * \note This function is `[[nodiscard]]` and `noexcept`.
 */
[[nodiscard]] auto contains_substring(std::string_view text,
 ...
 */
[[nodiscard]] auto
read_and_preprocess_file(const std::filesystem::path &file_name)
    -> PreprocessedFile;
} // namespace OpenSMOKEpp::Lexer
