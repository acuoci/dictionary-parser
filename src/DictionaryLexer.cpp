...
namespace OpenSMOKEpp::Lexer
{
	namespace
	{
		...
		[[nodiscard]] auto ascii_equal_case_insensitive(const char lhs, const char rhs) noexcept -> bool
		{
	...
	auto split_whitespace(const std::string_view text) -> std::vector<std::string_view>
	{
	...
	auto count_char(const std::string_view text, const char value) noexcept -> std::size_t
	{
		return static_cast<std::size_t>(std::count(text.begin(), text.end(), value));
	}

	auto count_substring(const std::string_view text, const std::string_view needle) noexcept -> std::size_t
	{
		if (needle.empty())
			return 0;
	...
	auto count_substring_case_insensitive(const std::string_view text, const std::string_view needle) noexcept -> std::size_t
	{
		...
		for (std::size_t i = 0; i <= text.size() - needle.size(); ++i)
		{
			...
			for (std::size_t j = 0; j < needle.size(); ++j)
				matched = matched && ascii_equal_case_insensitive(text[i + j], needle[j]);
			if (matched)
				++count;
		}
	...
	auto contains_substring(const std::string_view text, const std::string_view needle) noexcept -> bool
	{
	...
	auto read_and_preprocess_file(const std::filesystem::path& file_name) -> PreprocessedFile
	{
		std::ifstream input(file_name);
		if (!input.is_open())
			throw std::runtime_error("Unable to open dictionary file: " + file_name.string());
		...
		{
			line = std::string(strip_comment(line));
		...
		if (input.bad())
			throw std::runtime_error("I/O error while reading dictionary file: " + file_name.string());
